...
from clients.courses.CoursesClient import CoursesClient
...
class TestCourses:
    ...
    def test_create_course(
        ...
        #Присваиваем ID из фикстур в объект запроса
        request.preview_file_id = function_file.response.file.id
        request.created_by_user_id = function_user.response.user.id
        ...
        assert_status_code(response.status_code, HTTPStatus.OK)
        # Проверяем, что созданный курс соответствует отправленному запросу
